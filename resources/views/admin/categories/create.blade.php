        ...
        <form action="{{ route('categories.store') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('name') is-invalid @enderror" 
                       ...
                       placeholder="Contoh: Otomotif, Politik, Hiburan" required autofocus>
                @error('name')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                <small class="text-muted d-block mt-1">Slug URL akan dibuat otomatis berdasarkan nama kategori.</small>
            </div>
            <hr>
